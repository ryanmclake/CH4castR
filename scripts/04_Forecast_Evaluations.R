...
gelman <- ggplot(gelman_ebu_model, aes(forecast_date, mpsrf))+
  geom_line(size = 1.2)+
  geom_hline(yintercept = 1.01, lty = "dashed")+
  ylim(c(0.99,1.05))+
  theme_bw()+
...
one_week_forecast_wDA_eval <- trap_all_partition %>%
  select(time, mean, ebu_rate, forecast_date, weeks)%>%
  na.omit(.)%>%
  group_by(forecast_date)%>%
  filter(weeks == 1) %>%
  filter(forecast_date < as.Date("2019-11-07"))%>%
  ungroup(.)%>%
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "One-week with refitting")
...
two_week_forecast_wDA_eval <- trap_all_partition %>%
  ...
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "Two-week with refitting")

one_week_forecast_nDA_eval <- trap_static_partition %>%
  select(time, mean, ebu_rate, forecast_date, days)%>%
  na.omit(.)%>%
  group_by(forecast_date)%>%
  filter(days == 1) %>%
  filter(forecast_date < as.Date("2019-11-07"))%>%
  ...
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "One-week no refitting")
...
two_week_forecast_nDA_eval <- trap_static_partition %>%
  ...
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "Two-week no refitting")
...
one_week_forecast_null_eval <- trap_null_partition %>%
  ...
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "One-week persistence null")
...
two_week_forecast_null_eval <- trap_null_partition %>%
  ...
  summarize(NSE = NSE(mean, ebu_rate),
            RMSE = RMSE(mean, ebu_rate))%>%
  mutate(model = "Two-week persistence null")

table <- rbind(one_week_forecast_wDA_eval, two_week_forecast_wDA_eval,
               one_week_forecast_nDA_eval, two_week_forecast_nDA_eval,
               one_week_forecast_null_eval,two_week_forecast_null_eval)

pdf("./figures/Evalution_table.pdf", height=4, width=6)
grid.table(table)
dev.off()
...
paramter
ggsave(path = ".", filename = "./figures/FIGURE4_paramters.jpg", width = 10, height = 10, device='jpg', dpi=600)



#  PARTITION UNCERTATINY
c <- all_partitioned_melt%>%
  group_by(variable)%>% filter(day_in_future == 1) %>%
  ...
  theme_bw()+
  labs(title = "A: Partitioned uncertainty of each one-week forecast")+
...
ggsave(path = ".", filename = "./figures/FIGURE6_partition.jpg", width = 18, height = 12, device='jpg', dpi=400)
